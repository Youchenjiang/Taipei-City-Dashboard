	...
	(function (global, factory) {
		...
		var asm = (function (global, env, buffer) {
			...
			function ja(d, e, f, g, h) {
				...
				if (
					((j & -2) << 24) >> 24 == 20 ? (c[k >> 2] | 0) > (f | 0) : 0
				) {
					f = ((c[r >> 2] | 0) + 1) | 0;
					c[r >> 2] = f;
					t = (d + 244) | 0;
					b[((c[t >> 2] | 0) + (f << 4) + 12) >> 1] = l;
					c[((c[t >> 2] | 0) + (c[r >> 2] << 4) + 8) >> 2] =
						c[(s + 20) >> 2];
		...
		function mergeParagraphLineBreakPoints(
			lineBreakPoints,
			paragraphCount
		) {
		...
		function processBidirectionalText(input, lineBreakPoints) {
			var nDataBytes = (input.length + 1) * 2;
			var stringInputPtr = Module._malloc(nDataBytes);
			var paragraphCount = setParagraph(
				input,
				stringInputPtr,
				nDataBytes
			);
